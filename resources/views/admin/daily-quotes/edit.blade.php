...
@section('title', 'Modifier la Phrase du Jour')

@section('content')
<div class="content-header">
    <h1><i class="bi bi-quote"></i> Modifier la Phrase du Jour</h1>
</div>

<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.daily-quotes.update', $quote) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control @error('quote_date') is-invalid @enderror" 
                       id="quote_date" name="quote_date" value="{{ old('quote_date', $quote->quote_date->format('Y-m-d')) }}" required>
                @error('quote_date')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('quote_1') is-invalid @enderror" 
                          id="quote_1" name="quote_1" rows="2" required>{{ old('quote_1', $quote->quote_1) }}</textarea>
                @error('quote_1')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('quote_2') is-invalid @enderror" 
                          id="quote_2" name="quote_2" rows="2" required>{{ old('quote_2', $quote->quote_2) }}</textarea>
                @error('quote_2')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('quote_3') is-invalid @enderror" 
                          id="quote_3" name="quote_3" rows="2" required>{{ old('quote_3', $quote->quote_3) }}</textarea>
                @error('quote_3')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $quote->is_active) ? 'checked' : '' }}>
